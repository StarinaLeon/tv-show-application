...
export class ShowsEffects {
	...
	loadShows$: Observable<Action> = this.actions$.pipe(
		...
		mergeMap((action) =>
			this.httpService
				.getShows()
				.pipe(map((shows) => ({type: SHOWS_ACTION_TYPES.LOAD_SHOWS.SUCCEEDED, payload: shows})))
		)
	);
	...
	loadGenres$: Observable<Action> = this.actions$.pipe(
		...
		mergeMap((action) =>
			this.httpService
				.getGenres()
				.pipe(map((genres) => ({type: SHOWS_ACTION_TYPES.LOAD_GENRES.SUCCEEDED, payload: genres})))
		)
	);
	...
	loadYears$: Observable<Action> = this.actions$.pipe(
		...
		mergeMap((action) =>
			this.httpService
				.getYears()
				.pipe(map((years) => ({type: SHOWS_ACTION_TYPES.LOAD_YEARS.SUCCEEDED, payload: years})))
		)
	);
}
